...
| `SPEC-ADMIN-07` | `REQ-060`, `REQ-111`, `REQ-112`, `REQ-113`, `REQ-114`, `REQ-119` | `CR-05/Q16`, `CR-06/Q04`, `CR-06/Q11`, `CR-06/Q13`, `CR-06/Q14`, `CR-07/Q01` | `CASE-ADMIN-18`, `CASE-ADMIN-19` | `test_CASE_ADMIN_18_pointage_du_solde_sans_transaction`, `test_CASE_ADMIN_19_pointage_reversible_et_trace` | `343723c`, `33e9077`, `ba5848f`, `be32351` |
...
| `SPEC-BOOKING-12` | `REQ-017`, `REQ-111`, `REQ-112`, `REQ-117` | `CR-01/Q10`, `CR-06/Q04`, `CR-06/Q09`, `CR-07/Q01` | `CASE-BOOKING-40`, `CASE-BOOKING-41`, `CASE-BOOKING-42` | `test_CASE_BOOKING_40_solde_regle_en_ligne_en_une_transaction`, `test_CASE_BOOKING_41_solde_non_reglable_hors_fenetre`, `test_CASE_BOOKING_42_solde_nul_et_creneau_annule` | `4145596`, `28b6093`, `2a108be`, `33e9077`, `ba5848f`, `916d0a6` |
...
| Les 3 cas de bout en bout ne sont pas exécutés | J6 | les 76 cas de niveau domaine et application sont automatisés en PHPUnit à J7 ; `CASE-BOOKING-08`, `CASE-BOOKING-22` et `CASE-BOOKING-35` relèvent du troisième niveau, qui suppose une couche HTTP à piloter. L'échéance « au plus tard à J9 » que portait cette ligne n'était pas atteignable, et elle est corrigée plutôt que reconduite | **scénarios écrits à J9** dans `tests/BoutEnBout/`, le Gherkin des cas repris tel quel, et Behat 4 installé, seule branche qui accepte Symfony 8. Les 21 étapes sont rapportées `undefined` : rien n'est exécuté, et les scénarios portent `@socle-absent`, que `tools/traceability.sh` écarte pour que le fichier n'éteigne pas la rupture. Vérifié : sans l'étiquette le contrôle tombe à 2 ruptures, avec elle il en compte 5. Reste dû : les définitions d'étapes, le jour où un socle sera déployé |
...
| `SPEC-CANCEL-07` a été codée sans plan de délégation | J9 | les 27 spécifications qui donnent lieu à une production ont le leur, écrit avant le code ; les quatre `SPEC-NFR` sans plan sont l'objet de la ligne précédente, ce qui fait bien 32 spécifications. Celle-ci est née à J9 d'un trou constaté en fin de descente, et a été écrite **directement**, sans passer par un agent : il n'y avait donc rien à cadrer. En rédiger le plan après coup donnerait une prévision fabriquée à partir du résultat, ce qui est exactement ce que la colonne « écart » de J10 cherche à mesurer | assumé et déclaré. La spécification, son cas `CASE-CANCEL-25` et ses deux tests existent ; c'est le plan, et lui seul, qui manque |
...
| Programmer une sortie n'a aucune spécification | J9 | `ProgrammerUneSortie` porte deux règles réelles, le refus des baleines hors saison et l'unicité du naturaliste sur un créneau, toutes deux issues de `SPEC-BOOKING-02`. Mais **le geste du gérant lui-même n'a jamais été décrit par le client** : aucune spécification ne dit qui programme une sortie, quand, ni ce qui l'en empêche. Le service n'existe que pour monter le monde des tests, et `MondeDeTest` est son seul appelant hors démonstration | trouvé le 2026-08-20 en vérifiant un à un les identifiants que la commande de démonstration annonce. Déclaré plutôt que masqué derrière un renvoi commode à `SPEC-ADMIN-01`, qui est la connexion à l'espace de gestion et n'a rien à voir. À poser au client avec les cinq questions du §8 de `CR-07` |
